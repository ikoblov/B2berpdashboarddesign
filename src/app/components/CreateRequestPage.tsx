...
import { X, Plus, Building2, FileText, Calendar, MessageSquare, ChevronLeft } from "lucide-react";
...
export function CreateRequestPage() {
  ...
  const handleSave = () => {
    console.log({ clientId, objectId, jobOrderId, executionDate, managerId, comment, shiftTemplates });
    navigate("/requests");
  };

  const handleCancel = () => navigate("/requests");

  return (
    <div className="min-h-full bg-gray-50" style={{ fontFamily: "'Nunito Sans', sans-serif" }}>
      {/* Page Header */}
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        ...
        <h1 className="text-gray-900">Создать заявку</h1>
      ...
      <div className="px-6 py-6">
          ...
          <div className="grid grid-cols-2 gap-x-8 gap-y-6">
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="space-y-4">
                  <div>
                    <Label htmlFor="client" className="text-sm text-gray-700">Клиент</Label>
                  ...
                  <div>
                    <Label htmlFor="object" className="text-sm text-gray-700">Объект</Label>
                  ...
                  <div>
                    <Label htmlFor="job-order" className="text-sm text-gray-700">Заказ-наряд</Label>
              ...
              <div className="pt-6 border-t border-gray-200">
                ...
                <div className="space-y-4">
                  <div>
                    <Label htmlFor="execution-date" className="text-sm text-gray-700">Дата исполнения</Label>
                    <Input
                      id="execution-date"
                      type="date"
                      value={executionDate}
                  ...
                  <div>
                    <Label htmlFor="manager" className="text-sm text-gray-700">Ответственный</Label>
            ...
            <div className="space-y-6">
              <div>
                ...
                <div>
                  <Label htmlFor="comment" className="text-sm text-gray-700">Комментарий</Label>
                  <Textarea
                    id="comment"
                  ...
                  />
                  <p className="text-xs text-gray-500 mt-2">{comment.length} / 500 символов</p>
                </div>
              </div>
              ...
              <div className="pt-6 border-t border-gray-200">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <Button variant="outline" size="sm" onClick={() => setIsAddingTemplate(true)} className="gap-2">
                ...
                {shiftTemplates.length > 0 ? (
                  <div className="space-y-2">
                    {shiftTemplates.map((template, index) => (
                      <div key={template.id ?? index} className="p-3 bg-gray-50 rounded-lg border border-gray-200">
                        <div className="flex items-center justify-between">
                          <div className="text-sm text-gray-900">Шаблон {index + 1}</div>
                          <button
                            onClick={() => setShiftTemplates(shiftTemplates.filter((_, i) => i !== index))}
                  ...
                  <div className="p-6 bg-gray-50 rounded-lg border border-gray-200 border-dashed">
                    <div className="text-center">
                      ...
                      <p className="text-sm text-gray-600 mb-3">Шаблоны смен не добавлены</p>
                      <Button variant="outline" size="sm" onClick={() => setIsAddingTemplate(true)} className="gap-2">
                        <Plus className="w-4 h-4" />
        ...
        <div className="mt-6 flex items-center justify-between bg-white border border-gray-200 rounded-lg px-6 py-4">
          <p className="text-xs text-gray-500">После создания заявка будет иметь статус "Черновик"</p>
          <div className="flex items-center gap-3">
            <Button variant="outline" onClick={handleCancel}>Отмена</Button>
            <Button onClick={handleSave} className="gap-2">Создать заявку</Button>
          </div>
        </div>
      </div>
      ...
      <ShiftTemplateModal
        ...
        onOpenChange={(open) => {
          ...
          if (!open) {
            setShiftTemplates((prev) => [...prev, { id: Date.now() }]);
          }
        }}
        mode="create"
      />
